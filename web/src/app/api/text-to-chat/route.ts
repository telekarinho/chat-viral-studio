...
export async function POST(req: Request) {
  ...
  try {
    const story = geminiEnabled()
      ? normalizeStory(await geminiJSON(buildTextToChatPrompt(text, params)), params)
    ...
    return NextResponse.json({ story, source: geminiEnabled() ? 'gemini' : 'local' });
  } catch (err: any) {
